...
export const getLeadsByCampaign = async (req: Request, res: Response) => {
  try {
    ...
    const campaignLeads = await CampaignModel.findOne({
      _id: campaignId,
    }).populate("leads");
    console.log("Fetched leads for campaign:", campaignLeads?.leads);
    console.log("Status", campaignLeads?.leads[0]);
...
export const createSingleLead = async (req: Request, res: Response) => {
  try {
    ...
    const checkExistingLead = await LeadModel.findOne({
      email: email,
      realtorId: realtor._id,
    ...
    if (checkExistingLead) {
      return res.status(400).json({
        error: "Lead with this email already exists for the selected campaign",
      });
    }
